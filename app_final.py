...
# --- Extraction des features attendues ---
...
y_test = test_df_logreg["churn"]

# --- Harmonisation des colonnes ---
def harmonize_features(df, feature_list):
    missing = [c for c in feature_list if c not in df.columns]
    dfc = df.copy()
    for c in missing:
        dfc[c] = 0
    return dfc[feature_list]

# === SCALING + BALANCING POUR CHAQUE MODELE ===

# --- Pour logreg/NN (scaling + SMOTE sur le train) ---
X_train_logreg = harmonize_features(train_df_logreg, expected_features_logreg)
y_train_logreg = train_df_logreg["churn"]
scaler_logreg = StandardScaler().fit(X_train_logreg)
X_train_logreg_scaled = scaler_logreg.transform(X_train_logreg)

smote = SMOTE(random_state=42)
X_train_logreg_scaled_bal, y_train_logreg_bal = smote.fit_resample(X_train_logreg_scaled, y_train_logreg)

# --- Pour Random Forest, DT, CB, XGB, LGB, TabNet (ADASYN sur le train) ---
def prep_and_balance(df, features, y_name="churn"):
    X = harmonize_features(df, features)
    y = df[y_name]
    adasyn = ADASYN(random_state=42)
    X_bal, y_bal = adasyn.fit_resample(X, y)
    return X_bal, y_bal

X_train_rf, y_train_rf           = prep_and_balance(train_df_other, expected_features_rf)
X_train_dt, y_train_dt           = prep_and_balance(train_df_other, expected_features_dt)
X_train_cb, y_train_cb           = prep_and_balance(train_df_other, expected_features_cb)
X_train_xgb, y_train_xgb         = prep_and_balance(train_df_other, expected_features_xgb)
X_train_lgb, y_train_lgb         = prep_and_balance(train_df_other, expected_features_lgb)
X_train_tabnet, y_train_tabnet   = prep_and_balance(train_df_other, expected_features_tabnet)
X_train_tabnet = X_train_tabnet.astype(np.float32)

# --- X_test pour chaque modèle ---
...
# --- Interface Streamlit ---
st.set_page_config(page_title="Prédiction Churn Ooredoo", layout="wide")
...
# --- Mapping pour l'explication IA : supporte ndarray ou DataFrame
feature_map = {
